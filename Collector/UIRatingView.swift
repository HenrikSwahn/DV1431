...
class UIRatingView: UIView {
    ...
    internal var delegate: RatingViewDelegate?
     ...
     */
    @IBInspectable internal var emptyImage: UIImage? {
        didSet {
     ...
     */
    @IBInspectable internal var fullImage: UIImage? {
        didSet {
    ...
    /**
     Minimum rating.
     */
    @IBInspectable internal var minRating: Int  = 0 {
        didSet {
    ...
    /**
     Max rating value.
     */
    @IBInspectable internal var maxRating: Int = 5 {
        didSet {
    ...
    /**
     Minimum image size.
     */
    @IBInspectable internal var minImageSize: CGSize = CGSize(width: 5.0, height: 5.0)
    
    /**
     Set the current rating.
     */
    @IBInspectable internal var rating: Float = 0 {
        didSet {
            if rating != oldValue {
                self.refresh()
            }
     ...
     */
    @IBInspectable internal var editable: Bool = true
    
    /**
     Ratings change by 0.5. Takes priority over floatRatings property.
     */
    @IBInspectable internal var halfRatings: Bool = false
    
    /**
     Ratings change by floating point values.
     */
    @IBInspectable internal var floatRatings: Bool = false
    ...
    func handleTouchAtLocation(touchLocation: CGPoint) {
        ...
        if let delegate = self.delegate {
            delegate.ratingView(self, isUpdating: self.rating)
        }
    }
    ...
    override internal func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?) {
        // Update delegate
        if let delegate = self.delegate {
            delegate.ratingView(self, didUpdate: self.rating)
        }
    }
